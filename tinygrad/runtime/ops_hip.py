...
class HIPProgram(Program[HIPDevice]):
  ...
  def __call__(self, *args, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1), vals:tuple[int, ...]=(), wait=False, **kw):
    ...
    if not hasattr(self, "vargs"):
      fields = [(f'f{i}', hip.hipDeviceptr_t if j < len(args) else getattr(ctypes, f"c_int{dt.bitsize}"), o)
                for i,((_,j,_,_),(o,dt)) in enumerate(zip(self.signature, TinyELF.iter_sig(self.signature, 0, len(args))))]
    ...
    for i,a in enumerate(TinyELF.args(self.signature, args, vals)): self.c_args.__setattr__(f'f{i}', a)
